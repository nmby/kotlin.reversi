...
class WidthFirstPlayer(
    ...
    private inner class Node(
        ...
        //
        // 本来、コストのかかるリバーシ盤の評価処理はリーフノード（最先端ノード）のみで実施すればよいが、
        // 以下の理由から、途中ノードでも評価計算を行ってその都度親ノードに結果を遡及させるという方法を採用した。
        //   1.最後に一度に評価処理を行って時間切れになるリスクを減らすため
        //   2.ソース簡略化のため
        //   3.お勉強としてobservableを使いたいため
...
private object Evaluators {
    ...
    fun evaluateBoard5(board: Board, color: Color): Double {
        ...
        val weights: Array<Array<Int>> = arrayOf(
                arrayOf(10, 0, 7, 5, 5, 7, 0, 10),
                arrayOf(0, -5, 3, 2, 2, 3, -5, 0),
                arrayOf(7, 3, 8, 5, 5, 8, 3, 7),
                arrayOf(5, 2, 5, 0, 0, 5, 2, 5),
                arrayOf(5, 2, 5, 0, 0, 5, 2, 5),
                arrayOf(7, 3, 8, 5, 5, 8, 3, 7),
                arrayOf(0, -5, 3, 2, 2, 3, -5, 0),
                arrayOf(10, 0, 7, 5, 5, 7, 0, 10)
